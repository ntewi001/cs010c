...
void Tree::TreeSplit(Node* curr, const string &key){
    ...
    if(curr->parent != nullptr){// edge case for when the node has parents
        //promote middlechild to parent large key
        curr->parent->large = middleChild;
        ...
        //making middlechild parent of leftchild and right child
...
void Tree::insert(const string &key, Node* curr){
    ...
    if(key < curr->small){ //edge case for when key is smaller than both keys
        ...
        if(curr->countData == 1){ //check how many are in curr
        ...
        else if(curr->countData == 2 && key < curr->large){ // when key is bigger than both in curr
...
void Tree::remove(const string &key, Node* curr){
    ...
    if(search(key, curr)){
        //find the node contianing key
        curr = searchNode(key, curr);
        //if node is root
